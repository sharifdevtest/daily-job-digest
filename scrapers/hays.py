...
def scrape():
    """Scrapes active IT job listings from Hays Middle East."""
    jobs = []
    url = "https://www.hays.ae/jobs/technology"
    try:
        ...
        cards = soup.select("div.job-card, div.search-result, article")
        print(f"[Debug] Hays: {len(cards)} card containers found.")
        
        for card in cards:
            title_elem = card.select_one("h3 a, h2 a, a[class*='title']")
            snippet_elem = card.select_one("p.snippet, div.description, p")
